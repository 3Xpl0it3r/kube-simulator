...
// prepareAllNecessaryClusterCertificates is response for prepare some necessary ceitificetes that needed by k8s components
// if certificated existed on disk, then  return
// if cetigicated file not existed on disk, then create new one
func prepareAllNecessaryClusterCertificates(config *Config) error {
...
func prepareComponentClusterConfigs(clusterCfg *cluster.Config) error {
